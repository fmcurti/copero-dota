...
//
// Members are protected, not private: CoperoRankedRoom subclasses this
// adapter, replacing only identity (sessions instead of browser UUIDs), the
// directory kind, and the post-match rating write.
//
...
export type ConnState = { playerId: string; name: string; spectating: boolean } | null;

export class CoperoRoom extends Server<Env> {
  static options = { hibernate: true };

  room: RoomState = freshRoom();
  protected bundle: DataBundle | null = null;
  protected pool: Pack[] | null = null;
  protected sim: { beats: Beat[]; result: SimResult; forSeed: number | null } | null = null;
  /** In-memory on purpose: a hibernation wake clears the publication cache and
   *  the next event re-announces the Room. */
  protected directoryState: DirectoryPublicationState | null = null;
  ...
  /** Which room class this is — the directory links and probes by it. */
  ...
  protected sendError(conn: Connection, code: string, msg: string, fatal = false) {
    conn.send(
  ...
  protected ensureSim(): { beats: Beat[]; result: SimResult } {
  ...
  protected directoryAt(now: number, previous = this.directoryState) {
    return roomDirectory(
      {
        code: this.name,
        kind: this.directoryKind(),
  ...
  protected connectedIds(): Set<string> {
